...
vi.mock('../src/db/index.js', () => {
    ...
    return {
        userQueries: {
            findById: vi.fn(async (id) => mockUsers.get(id) || null),
            findByGithubId: vi.fn(async (githubId) => {
            ...
            findByGithubUsername: vi.fn(async (username) => {
                for (const user of mockUsers.values()) {
        ...
        machineQueries: {
            findById: vi.fn(async (id) => mockMachines.get(id) || null),
...
describe('GitHub OAuth via Nango', () => {
    ...
    describe('GET /api/auth/nango/login-session', () => {
        it('should create a new login session with session token', async () => {
            ...
            expect(nangoService.createConnectSession).toHaveBeenCalledWith(expect.objectContaining({ id: expect.any(String) }));
        });
        it('should return 500 if session creation fails', async () => {
            vi.mocked(nangoService.createConnectSession).mockRejectedValue(new Error('Nango API error'));
            const response = await request(app)
                .get('/api/auth/nango/login-session')
                .expect(500);
    ...
    describe('POST /api/auth/nango/webhook', () => {
        ...
        describe('Auth Webhook Processing', () => {
            ...
            it('should process auth webhook for github-runner integration', async () => {
                ...
                expect(nangoService.updateEndUser).toHaveBeenCalledWith('webhook-conn-1', expect.objectContaining({ id: 'webhook-user-id' }));
            });
            ...
            it('should generate UUID for endUser.id if not provided', async () => {
                ...
                expect(nangoService.updateEndUser).toHaveBeenCalledWith('auto-id-conn', expect.objectContaining({
                    id: expect.stringMatching(/^[0-9a-f-]{36}$/i),
                }));
            });
            it('should handle errors from getGithubUser gracefully', async () => {
                vi.mocked(nangoService.getGithubUser).mockRejectedValue(new Error('GitHub API error'));
                const response = await request(app)
                    .post('/api/auth/nango/webhook')
                    .set('x-nango-signature', 'sig')
                    .send({
